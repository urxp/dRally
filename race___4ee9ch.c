#include "drally.h"
	...
	extern byte ___184185h[];
	extern byte ___184189h[];
	extern byte ___184191h[];
	extern byte ___243cc0h[];
	extern byte ___243cc8h[];
	extern byte ___243cc4h[];
	extern byte ___243ccch[];
	extern byte ___184199h[];
	extern byte ___1e6f80h[];
	extern byte ___1841a1h[];
	extern byte ___1841a9h[];
...
void race___4ee9ch(void){
	...
	byte 	esp[0x18];
		...
		ST(0) = ST(0)+(double)F32(___184185h);
		ST(0) = ST(0)*F64(___184189h);
		ST(0) = ST(0)/F64(___184191h);
		eax = D(___243cc0h);
		D(___243cc8h) = eax;
		eax = D(___243cc4h);
		D(___243ccch) = eax;
		...
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)*F64(___184199h);
		FPUSH(F32(edx+___1e6f80h));
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)*ST(1);
		FPUSH(F64(___1841a1h));
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)/ST(1);
		d_tmp = ST(0); ST(0) = ST(3); ST(3) = d_tmp;
		ST(0) = ST(0)*F64(___1841a9h);
		ST(2) = ST(2)*ST(0); FPOP();
		ST(1) = ST(1)/ST(0); FPOP();
		eax = D(___243cc8h);
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
