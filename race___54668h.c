...
___54699h:
		...
		//sahf    
		//jae     near ___548e3h
		if(FPOP() >= (double)F32(edx+___1e6ed0h+0xb0)) goto ___548e3h;
		...
		eax = D(___243ca8h);
		if((B(edx+eax*4+___1e6ed0h+0x20)&1) == 0) goto ___548e3h;
		eax = rand_watcom106();
		D(esp+0x11c) = eax;
...
___548e3h:
		...
		FPUSH(F32(edx+___1e6ed0h+0x10e));
		FPUSH(F32(edx+___1e6ed0h+0xb0));
		ST(0) = (int)ST(0);
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
