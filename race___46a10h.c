#include "drally.h"

	extern byte ___196d98h[];
	extern byte ___183569h[];
	extern byte ___1a51d0h[];
	extern byte ___1a51d1h[];
	extern byte ___1a51d2h[];
	extern byte ___1a42b4h[];
	extern byte ___1a42b8h[];
	extern byte ___1a42bch[];
	extern byte ___1a54d0h[];
	extern byte ___243d80h[];
	extern byte ___1b1cd0h[];
	extern byte ___196e74h[];
	extern byte ___196e78h[];
	extern byte ___2432fch[];
	extern byte ___243898h[];
	extern byte ___1a42c0h[];
	extern byte ___1a42c4h[];
	extern byte ___1a42c8h[];
	extern byte ___18353dh[];
	extern byte ___18356dh[];
	extern byte ___183545h[];
	extern byte ___183575h[];
	extern byte ___18354dh[];
	extern byte ___243304h[];
	extern byte ___243308h[];
	extern byte ___2432f8h[];
	extern byte ___2432ech[];
	extern byte ___18357dh[];
	extern byte ___2432f4h[];
	extern byte ___2432f0h[];
	extern byte ___183555h[];
	extern byte ___183565h[];
	extern byte ___183535h[];
	extern byte ___18352dh[];
	extern byte ___243310h[];
	extern byte ___243314h[];
	extern byte ___243874h[];
	extern byte ___183585h[];
	extern byte ___18355dh[];
	extern byte ___243894h[];
	extern byte ___243878h[];
	extern byte ___243330h[];
	extern byte VGA13_ACTIVESCREEN[];


void __DISPLAY_SET_PALETTE_COLOR(dword b, dword g, dword r, dword n);
void dRally_Sound_setMasterVolume(dword vol);
void __VGA13_PRESENTSCREEN__(void);
void ___58c60h(void);

double dR_Math_sin(double);
double dR_Math_cos(double);

// LEAVING RACE SCREEN ANIMATION
void race___46a10h(void){

	double 	d_tmp;
	dword 	eax, ebx, ecx, edx, edi, esi, ebp, rr, gg, bb, nn;
	byte 	esp[0x60];


		//cmp     dword [___196d98h], byte 40h
		//jne     near ___46f3ah
		if(D(___196d98h) != 0x40) goto ___46f3ah;
		ecx ^= ecx;
		D(esp+0x5c) = ecx;
		FPUSH(F32(___183569h));
___46a41h:
		edx = D(esp+0x5c);
		edx = edx+2*edx;
		eax ^= eax;
		L(eax) = B(edx+___1a51d0h);
		D(esp+0x4c) = eax;
		FPUSH((short)W(esp+0x4c));
		ST(0) = ST(0)/ST(1);
		eax ^= eax;
		L(eax) = B(edx+___1a51d1h);
		D(esp+0x4c) = eax;
		FPUSH((short)W(esp+0x4c));
		ST(0) = ST(0)/ST(2);
		eax ^= eax;
		L(eax) = B(edx+___1a51d2h);
		D(esp+0x4c) = eax;
		FPUSH((short)W(esp+0x4c));
		ST(0) = ST(0)/ST(3);
		edi = D(esp+0x5c);
		edi++;
		ecx += 0xc;
		D(esp+0x5c) = edi;
		d_tmp = ST(0); ST(0) = ST(2); ST(2) = d_tmp;
		F32(ecx+___1a42b4h) = (float)FPOP();
		F32(ecx+___1a42b8h) = (float)FPOP();
		F32(ecx+___1a42bch) = (float)FPOP();
		if((int)edi < 0x100) goto ___46a41h;
		edx = ___1a54d0h;
		FPOP();
		eax = edx+0xc800;
		ebx ^= ebx;
		D(esp+0x24) = eax;
___46ab9h:
		esi = D(___243d80h);
		ecx = 0x100;
		esi += ebx;
		edi = edx;
		esi += 0xa0;
		edx += 0x100;
		memcpy(edi, esi, ecx);
		ecx = D(esp+0x24);
		ebx += 0x200;
		if(edx != ecx) goto ___46ab9h;
		eax = ___1a54d0h;
		edx = ___1a54d0h;
		ebx ^= ebx;
		eax += 0xfb00;
		edx += 0xc900;
		D(esp+0x2c) = eax;
___46b0dh:
		eax = D(___243d80h);
		ecx = 0x40;
		eax += ebx;
		edi = edx;
		esi = eax+0x60;
		edx += 0x40;
		memcpy(edi, esi, ecx);
		esi = D(esp+0x2c);
		ebx += 0x200;
		if(edx != esi) goto ___46b0dh;
		edx = ___1a54d0h;
		ebx = edx+0xc700;
		eax = edx+0x6400;
		D(esp+0x28) = eax;
___46b54h:
		ecx = 0x100;
		edi = ___1b1cd0h;
		esi = ebx;
		memcpy(edi, esi, ecx);
		ecx = 0x100;
		esi = edx;
		edi = ebx;
		memcpy(edi, esi, ecx);
		ecx = 0x100;
		esi = ___1b1cd0h;
		edi = edx;
		memcpy(edi, esi, ecx);
		edi = D(esp+0x28);
		edx += 0x100;
		ebx -= 0x100;
		if(edx != edi) goto ___46b54h;
		ebx = 0x100;
		eax = ___1b1cd0h;
		edx ^= edi;
		memset(eax, edx, ebx);
		ecx ^= ecx;
		eax = 0x3f666666;
		edx = 0x42b40000;
		ebx = 0x40;
		D(___196e74h) = eax;
		D(___196e78h) = edx;
		D(___2432fch) = ebx;
		D(___243898h) = ecx;
___46bf2h:
		esi ^= esi;
		D(esp+0x5c) = esi;
___46bf8h:
		FPUSH(F32(esi+___1a42c0h));
		eax ^= eax;
		ST(0) = ST(0)*(double)F32(___196e78h);
		L(eax) = B(esp+0x5c);
		ST(0) = (int)ST(0);
		nn = eax;
		D(esp+0x48) = (int)FPOP();
		L(eax) = B(esp+0x48);
		FPUSH(F32(esi+___1a42c4h));
		eax &= 0xff;
		ST(0) = ST(0)*(double)F32(___196e78h);
		rr = eax;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		L(eax) = B(esp+0x48);
		FPUSH(F32(esi+___1a42c8h));
		eax &= 0xff;
		ST(0) = ST(0)*(double)F32(___196e78h);
		gg = eax;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		L(eax) = B(esp+0x48);
		eax &= 0xff;
		edi = D(esp+0x5c);
		bb = eax;
		esi += 0xc;
		edi++;
		__DISPLAY_SET_PALETTE_COLOR(bb, gg, rr, nn);
		D(esp+0x5c) = edi;
		if((int)edi < 0x100) goto ___46bf8h;
		FPUSH(F32(___196e78h));
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		eax = 0x2d8*D(esp+0x48);
		dRally_Sound_setMasterVolume(eax);
		FPUSH(F32(___196e78h));
		ST(0) = ST(0)*F64(___18353dh);
		ST(0) = ST(0)/F64(___18356dh);
		FPUSH(ST(0));
		ST(0) = dR_Math_sin(ST(0));
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = dR_Math_cos(ST(0));
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)*F64(___183545h);
		ST(0) = F64(___183575h)/ST(0);
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)*F64(___18354dh);
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = (int)ST(0);
		D(___243304h) = (int)FPOP();
		edx = D(___243304h);
		ebx = VGA13_ACTIVESCREEN+0x40;
		eax = 4*edx;
		D(___243308h) = ebx;
		eax -= edx;
		ebx ^= ebx;
		eax <<= 3;
		ST(0) = (int)ST(0);
		eax += edx;
		D(___2432f8h) = (int)FPOP();
		eax <<= 3;
		edx = 0xc940;
		D(___2432ech) = eax;
___46d07h:
		FPUSH(F32(___196e78h));
		ST(0) = ST(0)/F64(___18357dh);
		edi = D(___243308h);
		esi = edx;
		edi -= 0x40;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		ecx = D(esp+0x48);
		edx += 0x40;
		esi -= ecx;
		edi += ebx;
		esi = ___1a54d0h+esi;
		ebx += 0x140;
		memcpy(edi, esi, ecx);
		if(edx != 0xfb40) goto ___46d07h;
		__VGA13_PRESENTSCREEN__();
		ecx ^= ecx;
		esi ^= esi;
___46d5ah:
		FPUSH(F32(___196e78h));
		ST(0) = ST(0)/F64(___18357dh);
		ebx = D(___2432fch);
		eax = D(___243308h);
		edx = esi;
		eax -= 0x40;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		edi = D(esp+0x48);
		eax += ecx;
		ebx -= edi;
		eax += edi;
		ecx += 0x140;
		memset(eax, edx, ebx);
		if(ecx != 0xfa00) goto ___46d5ah;
		FPUSH(F32(___196e78h));
		ST(0) = ST(0)/F64(___18357dh);
		ebx ^= ebx;
		D(esp+0x38) = ebx;
		ST(0) = (int)ST(0);
		D(___2432fch) = (int)FPOP();
___46db9h:
		eax = D(___2432ech);
		L(eax) = 0;
		D(___2432f4h) = eax;
		if((int)eax <= 0xc800) goto ___46dd6h;
		D(___2432f4h) = 0xc800;
___46dd6h:
		eax = D(___2432f4h);
		edi = D(___2432f8h);
		eax = eax*edi;
		edx = eax;
		ebx = D(___243308h);
		edx = (int)edx>>0x10;
		eax = (int)eax>>0x11;
		D(___2432f0h) = edx;
		edx = eax-0xa;
		ebx += edx;
		L(ecx) = 0xa;
		L(eax) = 0;
___46e01h:
		if((short)X(edx) < 0) goto ___46e08h;
		B(ebx) = L(eax);
___46e08h:
		ebx++;
		X(edx)++;
		L(ecx)--;
		if(L(ecx)) goto ___46e01h;
		edx = 0xff;
		eax = D(___2432f0h);
		ebx = D(___2432f4h);
		ecx = D(___243308h);
		ebx--;
		edi = eax;
		edx -= eax;
		edi = (int)edi>>0x1;
		ebx = ___1a54d0h+ebx;
		edi += ecx;
		ecx = 0x100;
___46e39h:
		L(eax) = B(ebx);
		H(edx) += L(edx);
		B(edi) = L(eax);
		edi += !!(H(edx) < L(edx));
		ebx++;
		ecx--;
		if(ecx) goto ___46e39h;
		edx = D(___2432f0h);
		edx = (int)edx>>0x1;
		esi = D(___2432f0h);
		edx += 0xff;
		ebx = D(___243308h);
		edx -= esi;
		ebx += edx;
		L(ecx) = 0xa;
		L(eax) = 0;
___46e68h:
		if((short)X(edx) > 0xff) goto ___46e78h;
		B(ebx) = L(eax);
		ebx++;
		X(edx)++;
		L(ecx)--;
		if(L(ecx)) goto ___46e68h;
___46e78h:
		ebx = D(esp+0x38);
		edi = D(___243308h);
		eax = D(___243304h);
		edx = D(___2432ech);
		edi += 0x140;
		ebx++;
		edx -= eax;
		D(___243308h) = edi;
		D(___2432ech) = edx;
		D(esp+0x38) = ebx;
		if((int)ebx < 0xc8) goto ___46db9h;
		if(D(___196e78h) == 0x3f800000) goto ___472cdh;
		edi ^= edi;
		D(esp+0x5c) = edi;
		if((int)D(___243898h) < 0) goto ___46f00h;
		FPUSH(F32(___196e74h));
		FPUSH(F64(___183555h));
___46eddh:
		FPUSH(ST(1));
		edx = D(esp+0x5c);
		FPUSH(ST(0));
		edx++;
		ST(0) = ST(0)*ST(2);
		D(esp+0x5c) = edx;
		ST(1) = ST(0); FPOP();
		ST(2) = ST(0); FPOP();
		if((int)edx <= (int)D(___243898h)) goto ___46eddh;
		FPOP();
		F32(___196e74h) = (float)FPOP();
___46f00h:
		FPUSH(F32(___196e74h));
		ST(0) = (double)F32(___196e78h)-ST(0);
		F32(___196e78h) = (float)FPOP();
		if((int)D(___196e78h) >= 0x3f800000) goto ___46f28h;
		D(___196e78h) = 0x3f800000;
___46f28h:
		edi ^= edi;
		D(___243898h) = edi;
		___58c60h();
		goto ___46bf2h;
___46f3ah:
		ebx ^= ebx;
		edx ^= edx;
		D(esp+0x5c) = ebx;
		FPUSH(F32(___183565h));
___46f48h:
		ebx = D(esp+0x5c);
		ebx = ebx+2*ebx;
		eax ^= eax;
		L(eax) = B(ebx+___1a51d0h);
		D(esp+0x4c) = eax;
		FPUSH((short)W(esp+0x4c));
		ST(0) = ST(0)/ST(1);
		eax ^= eax;
		L(eax) = B(ebx+___1a51d1h);
		D(esp+0x4c) = eax;
		FPUSH((short)W(esp+0x4c));
		ST(0) = ST(0)/ST(2);
		eax ^= eax;
		L(eax) = B(ebx+___1a51d2h);
		D(esp+0x4c) = eax;
		FPUSH((short)W(esp+0x4c));
		ST(0) = ST(0)/ST(3);
		esi = D(esp+0x5c);
		esi++;
		edx += 0xc;
		D(esp+0x5c) = esi;
		d_tmp = ST(0); ST(0) = ST(2); ST(2) = d_tmp;
		F32(edx+___1a42b4h) = (float)FPOP();
		F32(edx+___1a42b8h) = (float)FPOP();
		F32(edx+___1a42bch) = (float)FPOP();
		if((int)esi < 0x100) goto ___46f48h;
		FPOP();
		ebx = ___1a54d0h;
		edx ^= edx;
___46fb6h:
		ecx = 0x140;
		eax = D(___243d80h);
		edi = ebx;
		eax += edx;
		ebx += 0x140;
		esi = eax+0x60;
		edx += 0x200;
		memcpy(edi, esi, ecx);
		if(edx != 0x19000) goto ___46fb6h;
		eax ^= eax;
		edx ^= edx;
		D(esp+0x5c) = eax;
		FPUSH(F64(___183535h));
		FPUSH(F64(___18352dh));
___46fffh:
		FPUSH((int)D(esp+0x5c));
		ST(0) = ST(0)*ST(1);
		FPUSH(ST(0));
		ST(0) = dR_Math_cos(ST(0));
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = dR_Math_sin(ST(0));
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)*ST(3);
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = ST(0)*ST(3);
		ebx = D(___243310h);
		d_tmp = ST(0); ST(0) = ST(1); ST(1) = d_tmp;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		eax = D(esp+0x48);
		ST(0) = (int)ST(0);
		D(edx+ebx) = eax;
		ebx = D(___243314h);
		D(esp+0x48) = (int)FPOP();
		eax = D(esp+0x48);
		D(edx+ebx) = eax;
		ebx = D(esp+0x5c);
		ebx++;
		edx += 0x4;
		D(esp+0x5c) = ebx;
		if((int)ebx < 0x258) goto ___46fffh;
		FPOP();
		esi = 0x41000000;
		FPOP();
		D(esp+0x58) = esi;
___47064h:
		eax ^= eax;
		D(___243898h) = eax;
		eax = D(___243898h);
		D(___243874h) = eax;
		___58c60h();
		edx ^= edx;
		esi ^= esi;
		D(esp+0x5c) = edx;
___47082h:
		FPUSH(F32(esi+___1a42c0h));
		FPUSH(F32(esp+0x58));
		eax ^= eax;
		ST(0) = F64(___183585h)-ST(0);
		L(eax) = B(esp+0x5c);
		F64(esp+0x50) = FPOP();
		ST(0) = ST(0)*F64(esp+0x50);
		nn = eax;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		L(eax) = B(esp+0x48);
		FPUSH(F32(esi+___1a42c4h));
		eax &= 0xff;
		ST(0) = ST(0)*F64(esp+0x50);
		rr = eax;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		L(eax) = B(esp+0x48);
		FPUSH(F32(esi+___1a42c8h));
		eax &= 0xff;
		ST(0) = ST(0)*F64(esp+0x50);
		gg = eax;
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		L(eax) = B(esp+0x48);
		eax &= 0xff;
		bb = eax;
		__DISPLAY_SET_PALETTE_COLOR(bb, gg, rr, nn);
		ebx = D(esp+0x5c);
		ebx++;
		esi += 0xc;
		D(esp+0x5c) = ebx;
		if((int)ebx < 0x100) goto ___47082h;
		FPUSH(F64(esp+0x50));
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		eax = 0x1b4*D(esp+0x48);
		dRally_Sound_setMasterVolume(eax);
		FPUSH(F32(esp+0x58));
		ST(0) = (int)ST(0);
		D(esp+0x48) = (int)FPOP();
		eax = D(esp+0x48);
		ebx = 0x257;
		eax -= 0x8;
		ebx -= eax;
		eax = ebx;
		ebx = D(___243310h);
		ebx = D(ebx+4*eax);
		ebx += ebx;
		D(esp+0x44) = ebx;
		ebx = D(___243314h);
		eax = D(ebx+4*eax);
		eax += eax;
		D(esp+0x1c) = eax;
		eax = (int)eax>>0x1;
		D(esp+0x40) = eax;
		eax = D(esp+0x44);
		edx = VGA13_ACTIVESCREEN;
		eax = (int)eax>>0x1;
		edi ^= edi;
		D(esp+0x3c) = eax;
		D(esp+0x30) = eax;
		eax = D(esp+0x40);
		D(esp+0x34) = edi;
		D(esp+0x10) = eax;
___47187h:
		eax = D(esp+0x40);
		eax = (int)eax>>0xa;
		ebx = D(esp+0x40);
		D(esp+0x18) = eax;
		ebx += D(esp+0x1c);
		eax = D(esp+0x3c);
		ecx = D(esp+0x3c);
		eax = (int)eax>>0xa;
		D(esp+0x40) = ebx;
		D(esp+0x14) = eax;
		eax = D(esp+0x44);
		ebx = D(esp+0x30);
		ecx += eax;
		eax = D(esp+0x10);
		D(esp+0x3c) = ecx;
		D(esp+0x20) = eax;
		ecx ^= ecx;
___471c5h:
		esi = D(esp+0x18);
		edi = ebx;
		eax = D(esp+0x44);
		edi = (int)edi>>0xa;
		ebx += eax;
		eax = D(esp+0x20);
		edi -= esi;
		esi = D(esp+0x14);
		eax = (int)eax>>0xa;
		eax += esi;
		esi = D(esp+0x1c);
		D(esp+0x20) += esi;
		if((int)edi <= 0) goto ___47229h;
		if((int)edi >= 0x13f) goto ___47229h;
		if((int)eax <= 0) goto ___47229h;
		if((int)eax >= 0xc7) goto ___47229h;
		esi = eax;
		esi <<= 8;
		eax <<= 6;
		eax += esi;
		esi = eax+edi;
		esi = ___1a54d0h+esi;
		edi = edx;
		W(edi) = W(esi); edi += 2; esi += 2;
		edi += 0x13e;
		esi += 0x13e;
		W(edi) = W(esi); edi += 2; esi += 2;
		goto ___47238h;
___47229h:
		edi = edx;
		X(eax) = 0;
		W(edi) = X(eax); edi += 2;
		edi += 0x13e;
		W(edi) = X(eax); edi += 2;
___47238h:
		ecx += 0x2;
		edx += 0x2;
		if((int)ecx < 0x140) goto ___471c5h;
		edi = D(esp+0x34);
		edi += 0x2;
		edx += 0x140;
		D(esp+0x34) = edi;
		if((int)edi < 0xc8) goto ___47187h;
		__VGA13_PRESENTSCREEN__();
		edx ^= edx;
		D(esp+0x5c) = edx;
		if((int)D(___243898h) < 0) goto ___472a3h;
___47276h:
		if((int)D(esp+0x58) >= 0x43160000) goto ___47292h;
		FPUSH(F32(esp+0x58));
		FPUSH(ST(0));
		ST(0) = ST(0)*F64(___18355dh);
		ST(1) = ST(0); FPOP();
		F32(esp+0x58) = (float)FPOP();
___47292h:
		esi = D(esp+0x5c);
		esi++;
		D(esp+0x5c) = esi;
		if((int)esi <= (int)D(___243898h)) goto ___47276h;
___472a3h:
		FPUSH(F32(esp+0x58));
		//fcomp   dword [___183565h]
		//fnstsw  ax
		//sahf    
		//jbe     short ___472bah
		if(FPOP() <= (double)F32(___183565h)) goto ___472bah;
		D(esp+0x58) = 0x43160000;
___472bah:
		FPUSH(F32(esp+0x58));
		//fcomp   dword [___183565h]
		//fnstsw  ax
		//sahf    
		//jne     near ___47064h
		if(FPOP() != (double)F32(___183565h)) goto ___47064h;
___472cdh:
		edx ^= edx;
		D(___243898h) = edx;
		eax = D(___243898h);
		D(___243874h) = eax;
		D(___243894h) = edx;
		eax = D(___243878h);
		D(___243894h) = eax;
		eax = D(___243894h);
		D(___243330h) = eax;
		return;
}
